...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
  ...
  // Új kategória hozzáadásához csak másold le az egyik blokkot,
  // és ne felejtsd el frissíteni a TabController hosszát az initState-ben!
  ...
  // Nézetváltás és animációk állapotai
  int _currentPage = 0;
  ...
  void initState() {
    ...
    // 3. KATEGÓRIÁK SZÁMA: Ha változtatsz a kategóriákon, ezt a számot kell frissíteni!
  ...
  Widget _buildLoginForm(InputDecoration decoration, Map<String, Color> colors) {
    void submit() {
      if (_loginFormKey.currentState?.saveAndValidate() ?? false) {
        ...
        // Ide jön a bejelentkezési logika
      }
    }
          ...
          child: Column(
            ...
            children: [
              ...
              FormBuilderTextField(
                ...
                // JAVÍTVA: onFieldSubmitted -> onSubmitted
              ...
              FormBuilderTextField(
                ...
                decoration: decoration.copyWith(labelText: 'Jelszó', prefixIcon: Icon(Icons.lock_outline, color: colors['subtext']), suffixIcon: IconButton(icon: Icon(_isPasswordObscured ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: colors['subtext']), onPressed: () => setState(() => _isPasswordObscured = !_isPasswordObscured))),
                // JAVÍTVA: onFieldSubmitted -> onSubmitted
                onSubmitted: (_) => submit(),
              ),
  ...
  Widget _buildRegisterStepper(InputDecoration decoration, Map<String, Color> colors) {
    ...
    final step1Fields = [
      Padding(
        ...
        child: FormBuilderTextField(name: 'lastName', initialValue: _registrationData['lastName'], style: TextStyle(color: textColor), decoration: decoration.copyWith(labelText: 'Vezetéknév'), validator: FormBuilderValidators.required(errorText: 'Kötelező mező.')),
      ),
      const SizedBox(height: 16),
      Padding(
        padding: fieldPadding,
        child: FormBuilderTextField(
          ...
          validator: FormBuilderValidators.required(errorText: 'Kötelező mező.'),
          // JAVÍTVA: onFieldSubmitted -> onSubmitted
    ...
    final step2Fields = [
      Padding(
        padding: fieldPadding,
        child: FormBuilderTextField(name: 'username', initialValue: _registrationData['username'], style: TextStyle(color: textColor), decoration: decoration.copyWith(labelText: 'Felhasználónév'), validator: FormBuilderValidators.required(errorText: 'Kötelező mező.')),
      ),
      const SizedBox(height: 16),
      Padding(
        padding: fieldPadding,
    ...
    final step3Fields = [
      Padding(
        ...
        child: FormBuilderTextField(
          ...
          validator: FormBuilderValidators.compose([FormBuilderValidators.required(errorText: 'Kötelező mező.'), FormBuilderValidators.email(errorText: 'Érvénytelen formátum.')]),
          // JAVÍTVA: onFieldSubmitted -> onSubmitted
          onSubmitted: (_) => _nextPage(),
        ),
      ),
    ...
    final step4Fields = [
      Padding(
        padding: fieldPadding,
        child: FormBuilderTextField(name: 'password', style: TextStyle(color: textColor), obscureText: _isPasswordObscured, onChanged: _checkPasswordStrength, decoration: decoration.copyWith(labelText: 'Jelszó', suffixIcon: IconButton(icon: Icon(_isPasswordObscured ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: hintColor), onPressed: () => setState(() => _isPasswordObscured = !_isPasswordObscured)))),
      ),
      const SizedBox(height: 8),
      ...
      Padding(
        ...
        child: FormBuilderTextField(
          ...
          decoration: decoration.copyWith(labelText: 'Jelszó újra', suffixIcon: IconButton(icon: Icon(_isConfirmPasswordObscured ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: hintColor), onPressed: () => setState(() => _isConfirmPasswordObscured = !_isConfirmPasswordObscured))),
          validator: (val) => (val != _formKeys[3].currentState?.fields['password']?.value) ? 'A két jelszó nem egyezik.' : null,
          // JAVÍTVA: onFieldSubmitted -> onSubmitted
          onSubmitted: (_) {
    ...
    // Az összes lépés összeállítása
    final List<Widget> pages = [
      _buildStep("Add meg a neved", "Kérjük, a valós neved add meg.", _formKeys[0], step1Fields),
      _buildStep("Azonosítók", "Válassz egyedi azonosítókat.", _formKeys[1], step2Fields),
      _buildStep("Add meg az e-mail címed", "Ide küldjük az értesítéseket.", _formKeys[2], step3Fields),
      _buildStep("Válassz egy biztonságos jelszót", "Legalább 8 karakter, tartalmazzon kis- és nagybetűt, valamint számot.", _formKeys[3], step4Fields),
        ...
        child: Column(
          children: [
            ...
            Row(
              ...
              children: [
                if (_currentPage > 0) TextButton(onPressed: _isPageTransitioning ? null : _previousPage, child: const Text('Vissza', style: TextStyle(color: _primaryColor))) else const SizedBox(width: 60),
                const Spacer(),
                ElevatedButton(
                  ...
                  onPressed: _isPageTransitioning
                      ? null
                      : (_currentPage == pages.length - 1)
                          ? () {
                              _registrationData['avatar_id'] = _selectedAvatarId;
                              debugPrint("Regisztrációs adatok: $_registrationData");
                            }
          ...
          TabBar(
            ...
            onTap: (index) {
              ...
              if (firstIndexInCategory != -1) {
                pageController.animateToPage(firstIndexInCategory, duration: const Duration(milliseconds: 350), curve: Curves.easeOut);
              }
            },
          ),
          const SizedBox(height: 24),
          Expanded(
            child: PageView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final scale = isSelected ? 1.0 : 0.7;
                    ...
                    child: AnimatedOpacity(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: isSelected ? _primaryColor : Colors.transparent, width: 4),
                          boxShadow: isSelected
                              ? [BoxShadow(color: _primaryColor.withOpacity(0.5), blurRadius: 10, spreadRadius: 2)]
                              : [],
                        ),
                        child: CircleAvatar(
                          radius: 50,
                          backgroundColor: avatar['color'],
                          child: Icon(avatar['icon'], color: Colors.white, size: 45),
                        ),
                      ),
                    ),
                  ),
              ...
              onPageChanged: (index) {
                ...
                if (categoryIndex != -1 && _avatarTabController.index != categoryIndex) {
...
class PasswordStrengthIndicator extends StatelessWidget {
  ...
  String _getText(PasswordStrength s) {
    switch (s) {
      case PasswordStrength.weak: return 'Gyenge';
      case PasswordStrength.medium: return 'Közepes';
      case PasswordStrength.strong: return 'Erős';
      default: return 'Jelszó erőssége';
    }
  }
