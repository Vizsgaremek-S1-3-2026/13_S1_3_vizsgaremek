...
class PdfService {
  static Future<Uint8List> generateGradesReport({
    ...
    // 1. Load resources on the main thread
    Uint8List? logoBytes;
    try {
      final logoData = await rootBundle.load('assets/logo/logo_2.png');
    ...
    final params = {
      ...
      'options': options,
      'logoBytes': logoBytes,
      'fontBaseBytes': (fontBase as pw.TtfFont).data.buffer.asUint8List(),
      'fontBoldBytes': (fontBold as pw.TtfFont).data.buffer.asUint8List(),
    };

    // 3. Compute in background isolate
    return compute(_generatePdfRoot, params);
  }
}

// Top-level function for isolate
Future<Uint8List> _generatePdfRoot(Map<String, dynamic> params) async {
  final quizTitle = params['quizTitle'] as String;
  final groupName = params['groupName'] as String;
  final students = List<Map<String, dynamic>>.from(params['students']);
  final stats = Map<String, dynamic>.from(params['stats']);
  final quizData = params['quizData'] as Map<String, dynamic>?;

  final includeStats = params['includeStats'] as bool;
  final includeStudentList = params['includeStudentList'] as bool;
  final includeStudentDetails = params['includeStudentDetails'] as bool;
  final includeQuestions = params['includeQuestions'] as bool;
  final includeWarnings = params['includeWarnings'] as bool;
  final warningLayout = params['warningLayout'] as String;
  final options = Map<String, dynamic>.from(params['options']);

  final logoBytes = params['logoBytes'] as Uint8List?;
  final fontBaseBytes = params['fontBaseBytes'] as Uint8List;
  ...
  final fontBase = pw.Font.ttf(ByteData.view(fontBaseBytes.buffer));
  ...
  String _calculatePercent(dynamic score, dynamic max) {
    if (score == null || max == null || max == 0) return '0';
    final s = (score is int) ? score.toDouble() : (score as double);
    final m = (max is int) ? max.toDouble() : (max as double);
    return ((s / m) * 100).toStringAsFixed(0);
  }

  String formatTime() => DateFormat('yyyy-MM-dd HH:mm').format(DateTime.now());
  ...
  pw.Widget _buildTableCell(String text, {bool isHead = false}) {
    return pw.Padding(
      ...
      child: pw.Text(
        ...
        textAlign: isHead ? pw.TextAlign.center : pw.TextAlign.left,
      ),
    );
  }
  ...
  if (includeStudentDetails && quizData != null && quizData['blocks'] != null) {
    int idx = 0;
    for (var s in students) {
      if (s['status'] == 'submitted' ||
          s['status'] == 'closed' ||
          s['mock_answers'] != null) {
        final answers = s['mock_answers'] as List<Map<String, dynamic>>? ?? [];
              ...
              if (displayAnswers.isEmpty)
                ...
                ...displayAnswers.map((ans) {
                  ...
                  final borderColor = showCorrectness
                      ? (grayscale
                            ? PdfColors.grey600
                            : (correct ? PdfColors.green200 : PdfColors.red200))
                      : PdfColors.grey400;
    ...
    if (warningLayout == 'grouped') {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          footer: buildFooter,
          build: (c) => [
            pw.Text('ABC sorrendben', style: const pw.TextStyle(fontSize: 16)),
            pw.SizedBox(height: 10),
            ...statusList
                .map(
                  (s) => pw.Row(
